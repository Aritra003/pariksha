/**
 * Pariksha scoring run for eu.pariksha.eth against the European Union benchmark.
 * Engine v1.1.0 — scoreWithVariance(N=3). Mirrors scripts/score-seoul-kr.ts
 * but points at eu.pariksha.eth and european-union.json.
 *
 * Constraints:
 *   - NO on-chain calls.
 ...
 *
 * Bank source: pariksha-benchmark/questions/v1.1.0-draft/european-union.json
 */
...
const AGENT_ENS = 'eu.pariksha.eth'
const BANK_PATH = 'pariksha-benchmark/questions/v1.1.0-draft/european-union.json'
...
async function main() {
  ...
  if (flagged.length === 0) {
    console.log(`  No question has per-sample range > ${RANGE_FLAG_THRESHOLD}. Safe to --apply.`)
  } else {
    ...
    console.log(`  Decide on a prompt revision BEFORE --apply, not after.`)
  }
